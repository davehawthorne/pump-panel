...
modelComponents.hose = {
    fricLoss: function (resistance, lengths, parallel, flow) {
        return lengths * utils.sq(flow / parallel) * resistance;
    },
...
modelComponents.valve = function (midWayPresDrop, forFlow) {
    var priv = {
        opening: 0.0,
        halfwayResistance: midWayPresDrop / utils.sq(forFlow),
        resistance: function () {
            if (priv.opening === 0.0) {
    ...
    return {
        ...
        presDrop: function (flow) {
            return priv.resistance() * utils.sq(flow);
        },

        flowRate: function (presIn, presOut) {
            return utils.sqrt((presIn - presOut) / priv.resistance());
        },
        ...
        return {
            flow: function (pres) {
                ...
                if (pres < priv.minWorkingPres) {
                    return priv.setFlow * (1.0 - utils.sq(1.0 - pres / priv.minWorkingPres));
                } else {
                    return priv.setFlow;
                }
            },
